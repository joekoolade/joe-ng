...
- **THE BCM2711 HARDWARE RNG IS DRIVEN, AND `SecureRandom` SELF-SEEDS FROM IT (2026-09-22,
  ...
  - **THE DRIVER NEVER WRITES, AND THAT IS A SAFETY PROPERTY RATHER THAN LAZINESS.** The firmware has
    already enabled the block and let the FIFO fill, so there is nothing to configure -- and a guarded READ
    of an unmapped peripheral raises a fault this VM recovers from, while a STORE does not (the first probe
    of this arc died at an `<unclaimed pc>` the unwinder could not attribute). If `CTRL` ever reads as
    DISABLED this reports "no entropy" instead of enabling it: refusing is recoverable, guessing at the
    layout of a live peripheral is not.
  - **THE LIVENESS CHECK IS THE POINT, AND IT REJECTS RATHER THAN REPORTS.** `Rng.available()` draws sample
    words and refuses the source unless they VARY. A stuck-at line, a counter, or a window that reads back a
    constant all produce a perfectly stable stream that looks exactly like data -- and nothing downstream
    could ever tell, because every output of a random source looks equally correct. It probes once, latches,
    and DISCARDS its samples rather than feeding them to a caller.
  - **SEEDING IS ALL OR NOTHING.** A short read from the FIFO is treated exactly like no source at all.
    Stretching partial entropy over a full seed produces a weak seed that is indistinguishable from a strong
    one, so `require()` demands all 32 bytes or throws. The seed array is ZEROED after use -- it is the one
    piece of memory whose contents reproduce every byte the generator will ever emit.
  - **BAKED VM CODE CAN CATCH A HARDWARE DATA ABORT -- MEASURED, not assumed, and it was worth the boot.**
    The whole driver sits inside `try`/`catch (Throwable)`, and whether that works in WRITER-BAKED code (where
    `implicitChecks()` is false, so no null/bounds checks are emitted at all) was an open question. It does:
    under QEMU every register in the window faults and the driver returns "absent" with the boot intact.
    **The alternative was to HW-gate on `Uart.coreHz` like the WiFi path -- which would have worked and would
    have left the question permanently unanswered**, because QEMU would never have exercised the catch.
  - **AND THE FIRST QEMU RUN FAILED FOR A REASON I HAD WRITTEN A COMMENT ABOUT ONE LINE EARLIER.** A native
    is registered in `Loader.nativeBufAt` keyed by its DECLARING CLASS. I declared `hwEntropy0` on the nested
    `Sha1PrngSpi`, making that key `java/security/SecureRandom$Sha1PrngSpi` -- not what was registered. The
    diagnostic named it exactly (`LINK FAILED: ...$Sha1PrngSpi.hwEntropy0([BI)I -- class OK but no body for
    that name+descriptor`), which is the report earning its keep for the second time in two arcs. Moved to
    the outer class, with the reason recorded there.
  - **THE PROBE'S THREE ENTROPY ARMS NO LONGER ASSERT AN OUTCOME, BECAUSE THE OUTCOME IS NOW A PROPERTY OF
    THE BOARD.** With a hardware RNG present `nextBytes`, `generateSeed` and `getInstanceStrong` all succeed;
    without one they all refuse. Neither is a failure, so none is checked against a fixed expectation --
    **what IS asserted is that the three AGREE**. A VM whose `getInstanceStrong` hands back a generator that
    then refuses to produce bytes is incoherent on any board, and no single arm could see it. QEMU reads
    `false/false/false`, the host `true/true/true`, and both pass.
  - **PLUS THE ARM THAT CATCHES A SOURCE THAT WORKS AND IS WRONG:** where self-seeding succeeds, two FRESH
    generators must differ. A stuck source would seed every instance identically and still emit a perfectly
    random-looking stream -- the one failure no amount of looking at the output can catch. `BROKEN` is a
    suite marker now, and it reads 0.
  - **A CLASSPATH-ORDER MISTAKE OF MINE, recorded because it produced a CONFIDENT WRONG READING.** The host
...
- **THE LOCK'S CONTENTION IS MEASURED AT LAST -- 4 SOLO, 8 UNDER LOAD -- AND THE TWO CLEAN CONTENDED RUNS
  ...
  `[0 tests failed]`, closure byte-identical on every one:

  | arm | `lk:wait` | `yields` | `rel` | ms |
  |---|---|---|---|---|
  | solo | 4 | 138,040 | 4306 | 96,291 |
  | contended (suite QEMU alongside) | **8** | 144,971 | **4306** | 117,662 |

  - **THE LOADER LOCK IS GENUINELY CONTENDED, AND NOTHING COULD SAY SO BEFORE.** `rel` counts releases and
    `held` a duration; neither answers "did any task ever WAIT". That mattered because the guard that would
    otherwise answer it CANNOT: `checkCompileLockHeld` only reports entries where the lock is NOT ours, so
    once `loadAll` takes it, silence means "the lock worked" OR "nothing ever raced" and the two are
    indistinguishable. **Every clean contended run was therefore consistent with the experiment having no
    power at all.** `loaderWaits` counts OUTERMOST acquisitions that found ANOTHER task holding: four of
    them SOLO, i.e. four times a second task would have interleaved into the static context without the
    lock. 0.09% of 4,306 acquisitions -- rare, and real.
  - **`rel=4306` IS IDENTICAL IN BOTH ARMS**, so the same acquisitions did the same work and only the
    interleaving differed. That is what makes 4 -> 8 a reading rather than a side effect.
  - **EVERY WAIT LANDS BY BATCH 22 IN BOTH ARMS**, then nothing across the remaining 117. The `ProcessImpl`
    trap is at batch 21, so the contention sits exactly where this file already located the window --
    picocli's terminal-width thread against main -- and NOWHERE ELSE in the boot.
  - **AND IT CORRECTS THIS ARC'S FRAMING OF ITS OWN EXPERIMENT.** The second QEMU was set up as the thing
    that creates contention. It is not: the race window exists in the SOLO arm, generated by picocli's own
    thread. Load WIDENS it (4 -> 8) and does not create it -- so the solo arm was already a real test, and
    "reproduce it with a second QEMU" was never the necessary condition it was written as.
  - **TWO CLEAN CONTENDED RUNS ARE NOT EVIDENCE OF A FIX, AND THE ARITHMETIC IS THE POINT.** The card below
    records the failure as **2 in 4 contended runs**. Against a ~50% base rate, two passes in a row happen
    **one time in four by luck** -- so this is not significant, and the card below already warns in terms
    that "the original wording would have someone read a single clean loaded run as a fix". It is not read
    that way here. **Arm 3's post-fix failure (the `ClassReader.u1` NPE on a null `gbytes`) stands
    un-refuted**, and the g\* context is still reached from resolution paths no lock covers.
  - **`yields` ROSE ONLY 5% WHILE WAITS DOUBLED**, so the four extra waits were SHORT and the bulk of the
    yield count belongs to one or a few long waits present in both arms. Consistent with a waiter sitting
    through the `ProcessImpl` trap's spin until `loaderForceRelease` drops the hold -- which would make that
    release load-bearing rather than defensive. **NOT established:** an aggregate cannot separate one
    enormous wait from many small ones.
  - **A HARNESS BUG FOUND ON THE WAY, AND IT INVALIDATES RESULTS RATHER THAN JUST WASTING TIME.**
    `scripts/run-launcher.sh` broke its wait loop on `DENYLIST TRAP` -- which fires at batch 21 of EVERY
    healthy launcher boot -- so it killed QEMU 25s later, every time. **A log truncated at batch 21 reads
    exactly like a VM that died at batch 21**, and I read one that way before noticing the kill. Any
    launcher result produced THROUGH THIS SCRIPT since the trap was reclassified from boot-ender to "proof
    by PRESENCE" came from a truncated run. Second instance of this exact mistake in that one file; the
    grace period below the fix exists because an earlier version chopped stack traces off mid-frame.
  - **`taskExit` RELEASES THE LOADER LOCK NOW**, the same argument `loaderForceRelease` already makes for a
    halting trap, reached from the other permanent-stop path: the task parks BLOCKED on a never-posted
    semaphore and never runs again, so a hold it still owns is never handed back. Before `schedLock()`, not
    inside it -- `Magic.spinLock` is NOT recursive.
  - **THE STUCK-LOCK WATCHDOG PRINTS A VERDICT, not a state number this file records being misread three
    times.** RUNNABLE = a slow hold; BLOCKED = guest code under the lock (nothing INSIDE the compiler waits
    on another task, so an owner in that state is running a `<clinit>`); EXITED = stranded, checked FIRST
    via `taskDone` because `taskExit` parks BLOCKED too and the two states are otherwise identical.
    `gcPark` was checked and does not touch `taskState`, so a GC-parked compiler reads RUNNING and the
    verdict does not cry wolf. **UNEXERCISED on silicon** -- it needs a >10s stuck lock, so it is proven not
    to fire, not proven to work.
  - **THREE DIAGNOSTICS WERE READING A POINTER THEY THEMSELVES SAY IS UNTRUSTWORTHY.** `printCurrentClass`
    (the fault reporter AND the lock watchdog), `reportLocalsUndersized` and the new guard each did
    `printNameAt(gbase, gThisNameOff)`, which trusts the `u2` length it lands on. All three fire exactly
    when `gbase` may be clobbered -- `reportLocalsUndersized` IS the report the open bug prints, with raw
    constant-pool bytes for a class name. A garbage length is ~5.7 SECONDS of blocking UART plus a read past
    the blob. `printUtf8Capped` exists for this and records the previous occurrence; all three use it now.
    The collision report is capped too (it sits at `compile()`'s entry, once per METHOD).
  - **GATES:** demo suite on HARDWARE -- 33 programs, every new marker zero, `rounds=4 pend=180 reach=16`,
    `n:imap=52 synth=18 clinits=25`, `memo=1343 res=2478 unres=2224`, `sy:chg=0`, `smp sched: 4 of 4`,
    `steps/core 61/59/60/60`, `finish HML` / `HIGH blocked 60ms`, `churnMB=625 live=32 intact=32`,
    `lisp evals=600 result=610 stable=1`, `gc: collections=55`, WPA2 -> HTTP 200 OK. QEMU launcher x4 as
