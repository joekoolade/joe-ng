...
public final class CodeBuffer {
    ...
    public CodeBuffer()            { this(LOAD_ADDRESS); }
    public CodeBuffer(long base)   { this.base = base; }
    ...
    public void emitAll(List<Integer> ws) { for (int w : ws) emit(w); }

    /** Overwrite the word at index {@code i} — used to backpatch forward refs. */
    public void set(int i, int word) { words.set(i, word); }
    ...
    public long pcAt(int i) { return base + (long) i * 4; }

    /** Absolute PC of the next instruction to be emitted. */
    public long here() { return pcAt(words.size()); }
    ...
    public int[] toWords() {
        ...
        for (int i = 0; i < a.length; i++) a[i] = words.get(i);
        return a;
    }

    /** Raw little-endian bytes — this is the payload of {@code kernel8.img}. */
    public byte[] toBytes() { return A64.wordsToLittleEndian(toWords()); }
}
