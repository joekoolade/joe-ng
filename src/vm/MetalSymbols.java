...
final class MetalSymbols implements Symbols
{
    ...
    public boolean intrinsicEmitsCall(int methodCp)
    {
        ...
        return id == Intrinsics.CALL_N
            ...
            || id == Intrinsics.SET_PRIO || id == Intrinsics.GET_PRIO;
    }
    ...
    private static long helperAddr(int helper)
    {
        ...
        if (helper == Symbols.INTERRUPT)
        {
            return VM.interruptAddr;
        }
        if (helper == Symbols.IS_INTERRUPTED)
        {
            return VM.isInterruptedAddr;
        }
        if (helper == Symbols.CHECK_INTR)
        {
            return VM.checkIntrAddr;
        }
        ...
        if (helper == Symbols.SET_PRIO)
        {
            return VM.setPrioAddr;
        }
        if (helper == Symbols.GET_PRIO)
        {
            return VM.getPrioAddr;
        }
        ...
        if (helper == Symbols.SC_DOUBLE)
        {
            return VM.scDoubleAddr;
        }
        if (helper == Symbols.SC_FLOAT)
        {
            return VM.scFloatAddr;
        }
