...
of its boot inside `loadAll`**. Five increments took that to **10.09 seconds — 170x** — with `ALL PASSED`
...
| **total `loadAll`** | **1,711,850 ms** | **10,091 ms** | **170x** |
...
- **`pull`'s 1,309 ms is mostly UART, not computation.** It contains 448 `load` lines; the `NNus` values
  inside them sum to ~20 ms and `nameRegistered` is now an O(1) hash over 24,826 calls, so ~1.28 s of it is
  serial output at 115200 baud. Those lines print unconditionally -- every boot pays for them.
